...
| `rw_infant_1y`  | Yearly child mortality in Mexico      |
| `grw_infant_1y` | Yearly child mortality in Guadalajara |
| `grw_infant_1y` | Yearly child mortality in Guadalajara |
...
4.  Drop if `w_invterm` and `w_tmp_mean` are missing.
5.  Compute the monthly termal inversion (`w_invterm`) and the monthly temperature (`w_tmp_mean`).
6.  Replicate Figure 3 using `ggplot`. The mortality variables are `rw_infant_1y` and `grw_infant_1y`. Export to your `.tex` file.
...
2.  Replicate Table 3 (Columns 1 to 4 only). Export. Interpret. Are you confident with these results? WHich 
